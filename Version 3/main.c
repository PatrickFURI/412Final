...
void randomArray(int count, int ***arrayPtr)
{
	...
	*arrayPtr = malloc(count * sizeof(int *));
	...
	for(int i = 0; i < count; i++)
	{
		...
		} while(array != doorLoc ? pthread_mutex_trylock(posLocks[array[i][1]] + array[i][0])
				: inArray(array, i, robotLoc) || inArray(array, i, boxLoc) ||inArray(array, i, doorLoc));
	}
}
...
void moveRobot(unsigned robot, char dir)
{
	...
	{
		case 'N':
			if(robotLoc[robot][0] == boxLoc[robot][0] && robotLoc[robot][1] + 1 == boxLoc[robot][1]) {
				pthread_mutex_lock(posLocks[boxLoc[robot][1] + 1] + boxLoc[robot][0]);
				boxLoc[robot][1]++;
			        boxPushed = true;
			}
			else
				pthread_mutex_lock(posLocks[robotLoc[robot][1] + 1] + robotLoc[robot][0]);
			robotLoc[robot][1]++;
			pthread_mutex_unlock(posLocks[robotLoc[robot][1] - 1] + robotLoc[robot][0]);
			break;
		case 'S':
	                if(robotLoc[robot][0] == boxLoc[robot][0] && robotLoc[robot][1] - 1 == boxLoc[robot][1]) {
				pthread_mutex_lock(posLocks[boxLoc[robot][1] - 1] + boxLoc[robot][0]);
			...
			break;
		case 'E':
			if(robotLoc[robot][0] - 1 == boxLoc[robot][0] && robotLoc[robot][1] == boxLoc[robot][1]) {
				pthread_mutex_lock(posLocks[boxLoc[robot][1]] + boxLoc[robot][0] - 1);
				boxLoc[robot][0]--;
				boxPushed = true; 
			}
			else
				pthread_mutex_lock(posLocks[robotLoc[robot][1]] + robotLoc[robot][0] - 1);
			robotLoc[robot][0]--;
			pthread_mutex_unlock(posLocks[robotLoc[robot][1]] + robotLoc[robot][0] + 1);
			break;
		case 'W':
			if(robotLoc[robot][0] + 1 == boxLoc[robot][0] && robotLoc[robot][1] == boxLoc[robot][1]) {
				pthread_mutex_lock(posLocks[boxLoc[robot][1]] + boxLoc[robot][0] + 1);
...
void *solveRobot(void *arg)
{
	...
	boxLoc[robot][0] = -1;
	robotLoc[robot][0] = -1;
	pthread_mutex_unlock(posLocks[robotLoc[robot][1]] + robotX);
	pthread_mutex_unlock(posLocks[boxLoc[robot][1]] + boxX);
